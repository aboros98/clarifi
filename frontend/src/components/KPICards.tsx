  ...
  const cards = [
    {
      ...
      subtitle: actual?.runway_days ? `Runway: ${actual.runway_days} zile` : null,
      badge: actual?.bank_data_age_days && actual.bank_data_age_days > 3
        ? `Extras vechi: ${actual.bank_data_age_days} zile`
        : null,
      icon: Wallet,
    ...
    },
    {
      ...
      subtitle: receivables
        ? `${receivables.count} facturi · ${receivables.count_overdue} restante`
        : null,
      badge: receivables?.count_overdue > 0 ? `${receivables.count_overdue} restante!` : null,
      icon: Receipt,
    ...
    {
      title: "Proiecție 30 zile",
      value: expected ? `${fmt(expected.net_30d)} lei` : "—",
      subtitle: actual ? `Burn rate: ${fmt(actual.monthly_burn_rate)} lei/lună` : null,
      badge: null,
      icon: TrendingUp,
      bg: expected?.net_30d < 0
    ...
    },
    {
      ...
      subtitle: alerts
        ? `${alerts.critical} critice · ${alerts.warnings} avertismente`
        : null,
      badge: alerts?.critical > 0 ? "Atenție!" : null,
      icon: AlertTriangle,
      bg: alerts?.critical > 0
        ? "bg-red-50 border-red-200"
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
      {cards.map((card) => (
        <div key={card.title} className={`rounded-xl border p-4 sm:p-5 ${card.bg}`}>
          ...
          <div className={`text-xl sm:text-2xl font-bold ${card.text}`}>{card.value}</div>
          {card.subtitle && (
            <div className={`text-xs mt-1 ${card.text} opacity-60`}>{card.subtitle}</div>
          )}
          {card.badge && (
            <div className="mt-2">
